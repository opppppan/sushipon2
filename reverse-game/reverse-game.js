...
window.startReverseGame = function(screen) {
  ...
  function startEndingMovie() {
    ...
    // 7つの夢のフラッシュデータ
    const FLASHES = [
      { bg:'#f4eedd', label:'廊下',        textCol:'#4a2a10' },
      { bg:'#87ceeb', label:'空',          textCol:'#ffffff' },
      { bg:'#010e1e', label:'深海',        textCol:'#88ccff' },
      { bg:'#d4c870', label:'バックルーム', textCol:'#4a3a00' },
      { bg:'#000510', label:'夜空',        textCol:'#aaccff' },
      { bg:'#0d1a0d', label:'森',          textCol:'#88ffaa' },
      { bg:'#ffe8f8', label:'草原',        textCol:'#884466' },
    ];
    ...
    function movieLoop() {
      ...
      if(mt < FLASH_TOTAL){
        ...
        ctx.fillText(f.label, FIELD_W/2, FIELD_H/2+5);
        ctx.textAlign='left';
        ctx.globalAlpha=1;
      }
      // ── 暗転インターバル ──
  ...
  function autoDissolve() {
    const toRemove = world.bodies.filter(
      b => b.level === 0 && b.spawnProtect <= 0 && b.age > 50
    );
